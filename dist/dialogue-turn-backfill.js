import { getDialogueTurns, getRounds, getTokenUsageEvents, patchAutoSyncState, upsertDialogueTurn, } from "./local-storage.js";
...
function collectSeeds(rounds, tokenEvents, existingKeySet) {
    const seeds = [];
    for (const round of rounds) {
        const seed = roundToSeed(round);
        if (!seed)
            continue;
        if (existingKeySet.has(dialogueKey(seed.conversationId, seed.turnId)))
            continue;
        seeds.push(seed);
    }
...
function tokenEventToSeed(event) {
...
function hasStableTurnId(metadata) {
