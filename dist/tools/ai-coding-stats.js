...
export function registerAiCodingStatsTools(server, hooks = {}) {
    server.tool("begin_ai_dialogue_turn", "Force-start one dialogue turn. This captures a baseline up front so every conversation turn can be closed consistently later, even if no code changes happen.", {
    ...
    server.tool("list_ai_dialogue_baselines", "List unfinished dialogue baselines that are still present on disk.", {}, withLifecycle(hooks, async () => {
    ...
    server.tool("cleanup_ai_dialogue_baselines", "Delete stale dialogue baselines older than the given threshold.", {
    ...
    server.tool("end_ai_dialogue_turn", "Force-end one dialogue turn. If code changed since begin_ai_dialogue_turn, record an AI coding round; otherwise record dialogue token usage only.", {
        conversationId: z.string().min(1).describe("Stable id for the AI conversation/thread."),
        turnId: z.string().min(1).describe("Stable turn id for this dialogue turn."),
...
function hasTokenPayload(input) {
    return (input.totalTokens ?? (input.inputTokens ?? 0) + (input.outputTokens ?? 0)) > 0;
}
function normalizeSourceEventId(sourceEventId, conversationId, turnId, endedAt, client) {
...
async function computeMcpCodeStats(conversationId, projectPath, turnId) {
