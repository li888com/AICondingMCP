...
import { cleanupRoundBaselines, createCodeSnapshot, deleteRoundBaseline, findGitRoot, getCodeStatsSinceSnapshot, listRoundBaselines, loadRoundBaseline, saveRoundBaseline, } from "./code-stats.js";
import { recordDialogueTokenUsage, recordRound } from "./database.js";
import { getAutoSyncState, backupStorage, getStorageInfo, getRounds, getRoundReverts, getDialogueTurns, getTokenUsageEvents, getTokenUsageCandidates, getTokenUsageCandidate, patchAutoSyncState, upsertDialogueTurn, updateRound, updateTokenUsageCandidate, createTokenUsageEvent, createAiCodingCorrection, } from "./local-storage.js";
...
async function turnCommand(argv) {
...
async function dialogueTurnsCommand(argv) {
    const parsed = parseOptions(argv);
...
async function bindTokenCandidate(argv) {
    const parsed = parseOptions(argv);
...
async function markTokenUnavailable(argv) {
    const parsed = parseOptions(argv);
...
async function listPendingTokens(argv) {
    const parsed = parseOptions(argv);
    const limit = Number(parsed.limit ?? "50");
    const maxRows = Number.isSafeInteger(limit) && limit > 0 ? limit : 50;
...
async function listTokenCandidates(argv) {
    const parsed = parseOptions(argv);
...
async function loadSyncConfig() {
    ...
    const loaded = configs.filter((item) => Boolean(item));
    const merged = loaded.reduce((acc, item) => ({ ...acc, ...item.parsed }), {});
    const apiBaseUrl = stringValue(process.env.AI_CODING_REPORT_API_BASE_URL)
        ...
        ?? "http://127.0.0.1:9906";
    const turnApiPath = normalizeApiPath(stringValue(process.env.AI_CODING_TURN_API_PATH)
        ?? stringValue(process.env.SYNC_TURN_API_PATH)
        ?? stringValue(merged.turnApiPath)
        ?? "/ai-codingTurns");
    const hasAuth = Boolean(stringValue(process.env.SYNC_API_TOKEN)
...
function checkSyncConfig(config) {
    return {
        name: "sync config",
...
function checkTurnApiPath(config) {
    return {
        name: "turnApiPath",
...
function checkAuthConfig(config) {
    return {
        name: "sync auth",
        ok: config.hasAuth,
...
async function validateAutoRunnerProcess(pid, workerId, startedAt) {
    ...
    const isAutoRunnerCommand = normalizedCommand.includes(expectedDistScript) ||
...
function currentJobStartedAt(state) {
    if (!state?.currentStep)
        return null;
    if (state.currentStep.includes("token"))
        return state.lastTokenSyncStartedAt ?? null;
    if (state.currentStep.includes("online"))
        return state.lastOnlineSyncStartedAt ?? null;
    return null;
}
function isCurrentJobStale(state) {
    if (!state?.currentStep || state.currentStatus !== "running" || !state.lastHeartbeatAt)
        return false;
...
function defaultConversationIdForClient(projectPath, client) {
    const normalized = resolve(projectPath).replaceAll("\\", "/");
...
function buildDialogueTurnSummary(dialogueTurns, rounds, tokenEvents) {
...
function isUploadPending(item) {
    return !item._sync || item._sync.status === "pending";
}
function isUploadFailed(item) {
    return item._sync?.status === "failed";
}
function isUploadSynced(item) {
    return item._sync?.status === "synced";
}
function isUploadSkipped(item) {
