...
type AutoRunnerState = {
  ...
  idleBaseline: ActiveRound | null;
  lastSignature: string | null;
  lastChangedAt: string | null;
  lastSyncAt: string | null;
  recordedSignatures: string[];
};
...
async function recordDialogueTurn(gitRoot: string): Promise<void> {
...
async function tick(gitRoot: string): Promise<void> {
...
async function createBaseline(gitRoot: string, signature: string): Promise<ActiveRound> {
...
async function finishRound(state: AutoRunnerState, gitRoot: string, signature: string): Promise<void> {
...
async function getWorkspaceSignature(gitRoot: string): Promise<string> {
...
function isStorageStatusLine(gitRoot: string, line: string): boolean {
...
async function syncOnline(): Promise<void> {
  await patchAutoSyncState({
...
function shouldSync(lastSyncAt: string | null, now: Date): boolean {
  if (!lastSyncAt) return true;
  return now.getTime() - new Date(lastSyncAt).getTime() >= args.syncIntervalMs;
}

async function loadState(): Promise<AutoRunnerState> {
  ...
  const parsed = JSON.parse(raw) as Partial<AutoRunnerState>;
  return {
...
async function saveState(state: AutoRunnerState): Promise<void> {
...
function parseClient(value: string | undefined): TokenClient {
